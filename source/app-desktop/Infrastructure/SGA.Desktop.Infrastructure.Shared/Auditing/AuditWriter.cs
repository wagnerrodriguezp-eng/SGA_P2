...
namespace SGA.Desktop.Infrastructure.Shared.Auditing;

// Writes to the AuditLog table under normal conditions. If the database itself is unreachable,
// falls back to a rolling local text log so the event is never silently lost — same table as
// app-web-mvc, so both apps' actions show up in one unified audit trail.
